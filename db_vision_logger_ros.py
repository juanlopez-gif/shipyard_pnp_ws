...
from conveyor_detector import ConveyorDetector, FlowFeatureExtractor
...
# REALITY podia leerse mal justo en el instante del PLACE. Anadido aqui el
...
# ── Conveyor2 pick joints publisher ───────────────────────────────────────────
PICK_JOINTS_TOPIC_OUT   = "/robot2/pick_joints_c2s2"
Z_PICK_M                =  0.129      # FK pick_c2s2
Z_PREPICK_M             =  0.175      # FK prepick_c2s2 (se recalcula al arrancar via subprocess)
ROLL_PICK               = -2.647
PITCH_PICK              =  1.506
YAW_PICK                =  1.975
HOMOGRAPHY_C2_PATH      = BASE_DIR / "aruco_homography" / "homography.npy"
PICK_JOINTS_C2S2        = [-1.7129, -0.4277, -0.6037, -0.0597, -0.5953, -0.0014]
PREPICK_JOINTS_C2S2     = [-1.7190, -0.2611, -0.5734, -0.1226, -0.7794, -0.5936]
C2S2_DYNAMIC_PICK_SETTLE_SEC = float(os.environ.get("C2S2_DYNAMIC_PICK_SETTLE_SEC", "1.0"))
...
# Conveyor motion ML
# Solo conveyor1/2 publican topic dedicado; conveyor3/4 se leen de
# resources.conveyors en /factory/system_state (ver _on_system_state).
...
CONVEYOR_ML_MODELS = {
    ...
    "conveyor3": Path(os.environ.get("ML_CONVEYOR3_MODEL", str(BASE_DIR / "conveyor_model_c3"))),
    "conveyor4": Path(os.environ.get("ML_CONVEYOR4_MODEL", str(BASE_DIR / "conveyor_model_c4"))),
}
CONVEYOR_ML_CAMS = {
    "conveyor1": 6,
    "conveyor2": 4,
...
def build_supervisor_panel(supervisor_data: dict, verify_state: dict,
    ...
    if motion_expected and motion_detected:
        ...
        cv2.line(panel, (x0, y), (PANEL_WIDTH - x0, y), (50, 50, 50), 1)
        y += 8
        for mkey in ("conveyor1", "conveyor2", "conveyor3", "conveyor4"):
...
class VisionLoggerNode(Node):
    def __init__(self):
        ...
        self._display_stack_slots = {c: None for c in SLOT_COLS}

        # ── Modo entrenamiento inline (tecla T) ───────────────────
        # Estados: "idle" → "select" → "ready" ↔ "recording" → "training" → "idle"
        self._train_mode      = "idle"
        self._train_key       = None    # "conveyor1" .. "conveyor4"
        self._train_label     = None    # 0=STOPPED 1=RUNNING
        self._train_extractor = FlowFeatureExtractor()
        self._train_clip_end  = 0.0
        self._train_clip_sec  = 1.0
        self._train_last_t    = 0.0
        self._train_counts    = [0, 0]  # [stopped, running]
        ...
        # Todos los conveyors que tienen detector ML; conveyor3/4 reciben
        # el EXPECTED desde _on_system_state en vez de topic dedicado.
        _all_motion_keys = list(CONVEYOR_ML_MODELS.keys())
        self._motion_expected = {k: None for k in _all_motion_keys}
        self._motion_detected = {k: None for k in _all_motion_keys}
        self._motion_mismatch = {k: False for k in _all_motion_keys}
        self._motion_mismatch_since = {k: None for k in _all_motion_keys}
        ...
        # ── Conveyor2 pick joints ────────────────────────────────────
        self._robot2_at_capture            = False
        self._pick_joints_busy             = False
        self._cam4_c2_centroid             = None   # (cx, cy) ultimo centroide deteccion conveyor2
        self._last_robot2_c2s2_command_id  = None

        _H = None
        if HOMOGRAPHY_C2_PATH.exists():
            try:
                _H = np.load(str(HOMOGRAPHY_C2_PATH))
                self.get_logger().info("[C2S2] Homografía cargada.")
            except Exception as _e:
                self.get_logger().warn(f"[C2S2] Error cargando homografía: {_e}")
        else:
            self.get_logger().warn(f"[C2S2] Homografía no encontrada: {HOMOGRAPHY_C2_PATH}")
        self._H_c2 = _H

        self._pick_joints_pub = self.create_publisher(String, PICK_JOINTS_TOPIC_OUT, 10)
        self.create_subscription(String, "/niryo_factory/status",
                                 self._on_niryo_factory_status, 10)
        # resource_id correcto confirmado por shipyard_pnp: "vision_robot2"

        threading.Thread(target=self._compute_z_prepick, daemon=True).start()
    ...
    def _on_system_state(self, msg: String):
        ...
        with self._lock:
            ...
            # Grupo 1: bloqueado SIEMPRE que xarm2 tenga cualquier ciclo
            # activo -- sin early_unblock. El stack solo se desbloquea cuando
            # xarm2 queda realmente IDLE (sin tarea). El brazo puede estar
            # obstruyendo la camara en cualquier punto del ciclo (pick,
            # travel, place, return), no solo en el instante del pick.
            xarm2_any_active = bool(active.get("xarm2"))
            xarm2_busy = xarm2_any_active
            ...
            if prev_busy and not xarm2_busy:
                ...
                self.get_logger().info("[G1] xArm2 quedo IDLE: stack ACTIVO + snapshot")
            ...
            # Estado RUNNING/STOPPED de conveyor3 y conveyor4 desde
            # resources.conveyors — mismo JSON, sin topic adicional.
            _conv_res = resources.get("conveyors", {})
            for _ck in ("conveyor3", "conveyor4"):
                _st = _conv_res.get(_ck)
                if isinstance(_st, str):
                    self._motion_expected[_ck] = _st.upper()
    ...
    def _camera_loop(self):
        ...
        while self._running:
            ...
            for cam_idx, cap in self._caps.items():
                ...
                # ── Conveyor2 centroid tracking (cam 4) ──────────
                if cam_idx == 4:
                    _best_c2_conf = 0.0
                    _best_c2_ctr  = None
                    _best_c2_cls  = None
                    _best_c2_box  = None
                    for _box in boxes:
                        _bconf = float(_box.conf[0])
                        if _bconf < CONF_THRESHOLD:
                            continue
                        _bx1, _by1, _bx2, _by2 = _box.xyxy[0].tolist()
                        _bcx, _bcy = (_bx1+_bx2)/2, (_by1+_by2)/2
                        for _roi in cam_rois:
                            if (_roi["name"] == "conveyor2"
                                    and point_in_polygon(_roi["pts"], _bcx, _bcy)
                                    and _bconf > _best_c2_conf):
                                _best_c2_conf = _bconf
                                _best_c2_ctr  = (_bcx, _bcy)
                                _best_c2_cls  = names[int(_box.cls[0])]
                                _best_c2_box  = (_bx1, _by1, _bx2, _by2)
                    with self._lock:
                        self._cam4_c2_centroid = _best_c2_ctr

                    # Overlay mm coords en cam4
                    if _best_c2_ctr is not None and self._H_c2 is not None:
                        _pt = self._H_c2 @ np.array([_best_c2_ctr[0], _best_c2_ctr[1], 1.0])
                        _pt /= _pt[2]
                        _xmm, _ymm = float(_pt[0]), float(_pt[1])
                        _cx_i, _cy_i = int(_best_c2_ctr[0]), int(_best_c2_ctr[1])
                        _bx1, _by1, _bx2, _by2 = _best_c2_box
                        cv2.circle(frame, (_cx_i, _cy_i), 7, (0, 255, 180), -1)
                        cv2.putText(frame,
                                    f"{_best_c2_cls}  ({_xmm:.1f}, {_ymm:.1f}) mm",
                                    (_cx_i + 10, _cy_i - 10),
                ...
                if (self._train_mode == "recording"
                        and self._train_key is not None
                        and cam_idx == CONVEYOR_ML_CAMS.get(self._train_key)):
                    _det = self._motion_detectors[self._train_key]
                    _now_t = time.time()
                    _dt = (_now_t - self._train_last_t) if self._train_last_t > 0 else 1 / 15
                    self._train_last_t = _now_t
                    _feat = self._train_extractor.extract(_det._crop(frame), _dt)
                    if _feat is not None:
                        _det.classifier.add_sample(_feat, self._train_label)
                        self._train_counts[self._train_label] += 1
                    if _now_t >= self._train_clip_end:
                        self._train_mode = "ready"
                        self._train_extractor.reset()
                        _lbl = "STOPPED" if self._train_label == 0 else "RUNNING"
                        self.get_logger().info(
                            f"[Train] clip {_lbl} OK — "
                            f"stopped={self._train_counts[0]} running={self._train_counts[1]}"
                        )
            ...
            if xarm2_idle and slots != self._display_stack_slots:
                self._display_stack_slots = dict(slots)
            panel = build_supervisor_panel(_sup, _ver, self._last_reality, mosaic.shape[0],
                                           self._motion_expected, self._motion_detected,
                                           self._motion_mismatch, self._display_stack_slots)
            mosaic = np.hstack([mosaic, panel])

            # ── Overlay entrenamiento ─────────────────────────────
            if self._train_mode != "idle":
                self._draw_train_overlay(mosaic)
            if self._train_mode == "training" and self._train_thread \
                    and not self._train_thread.is_alive():
                self._train_thread = None
                self._train_mode   = "idle"
            ...
            # ── Teclas ───────────────────────────────────────────
            if key == ord('q') and self._train_mode == "idle":
                self._running = False
                break

            elif key in (ord('t'), ord('T')):
                if self._train_mode == "idle":
                    self._train_mode = "select"
                elif self._train_mode in ("select", "ready", "recording"):
                    self._train_mode = "idle"
                    self._train_key  = None

            elif self._train_mode == "select":
                _cmap = {ord('1'): "conveyor1", ord('2'): "conveyor2",
                         ord('3'): "conveyor3", ord('4'): "conveyor4"}
                if key in _cmap:
                    self._train_key    = _cmap[key]
                    self._train_counts = [0, 0]
                    self._train_extractor.reset()
                    self._train_mode   = "ready"
                elif key == 27:
                    self._train_mode = "idle"

            elif self._train_mode == "ready":
                if key == ord('1'):
                    self._start_train_clip(0)
                elif key == ord('2'):
                    self._start_train_clip(1)
                elif key in (13, ord('\r')):  # ENTER
                    total = sum(self._train_counts)
                    if total < 20:
                        self.get_logger().warn(
                            f"[Train] Solo {total} muestras — necesitas al menos 20 en total"
                        )
                    else:
                        self._launch_train()
                elif key == 27:
            ...
            elif self._train_mode == "idle" and key == ord(' '):
                with self._lock:
                    self._trigger_stack = True
                    self._blocked_stack = False
    ...
    def _start_train_clip(self, label: int):
        self._train_label    = label
        self._train_clip_end = time.time() + self._train_clip_sec
        self._train_last_t   = 0.0
        self._train_extractor.reset()
        self._train_mode = "recording"

    def _launch_train(self):
        key = self._train_key
        det = self._motion_detectors[key]
        ...
        def _do():
            ...
            import numpy as _np
            try:
                # Snapshot de datos (no bloqueamos el hilo de cámara)
                X_snap = list(det.classifier._X)
                y_snap = list(det.classifier._y)
                if len(X_snap) < 20:
                    raise ValueError(f"Solo {len(X_snap)} muestras — necesitas al menos 20")

                X = _np.vstack(X_snap)
                y = _np.array(y_snap)

                # Entrenamos un pipeline NUEVO — nunca tocamos el pipeline vivo
                new_pipe = _SKPipe([
                    ("scaler", _SS()),
                    ("clf", _GBC(n_estimators=200, max_depth=4,
                                 learning_rate=0.05, subsample=0.8, random_state=42)),
                ])
                scores = _cvs(new_pipe, X, y, cv=5, scoring="f1")
                new_pipe.fit(X, y)

                # Swap atómico: la asignación de referencia en Python es atómica (GIL)
                # El hilo de cámara verá el pipeline viejo completo o el nuevo completo,
                # nunca uno a medio entrenar.
                det.classifier.pipeline = new_pipe
                det.classifier.trained  = True
                det.save()

                self.get_logger().info(
                    f"[Train] {key}: F1={scores.mean():.3f}±{scores.std():.3f}"
                    f"  ({len(y)} muestras)"
                )
            except Exception as exc:
                self.get_logger().error(f"[Train] {key}: {exc}")
            finally:
                det.extractor.reset()

        self._train_thread = threading.Thread(target=_do, daemon=True)
        self._train_thread.start()

    def _draw_train_overlay(self, img: np.ndarray):
        now  = time.time()
        font = cv2.FONT_HERSHEY_SIMPLEX
        h, w = img.shape[:2]

        if self._train_mode == "select":
            bw, bh = 460, 300
            bx = (w - bw) // 2
            by = (h - bh) // 2
            ov = img.copy()
            ...
            cv2.putText(img, "MODO ENTRENAMIENTO",
                        (bx + 18, by + 34), font, 0.75, (0, 220, 255), 2)
            cv2.putText(img, "Selecciona conveyor:",
                        (bx + 18, by + 62), font, 0.52, (160, 160, 160), 1)
            opts = [
                ("1", "conveyor1", 6),
                ("2", "conveyor2", 4),
                ("3", "conveyor3", 2),
            ...
            for i, (k, name, cam) in enumerate(opts):
                cv2.putText(img, f"  {k}  {name}   (cam {cam})",
                            (bx + 18, by + 98 + i * 36), font, 0.58, (80, 255, 80), 1)
            cv2.putText(img, "  ESC  cancelar",
                        (bx + 18, by + 270), font, 0.52, (120, 80, 80), 1)

        elif self._train_mode in ("ready", "recording"):
            cam_idx = CONVEYOR_ML_CAMS.get(self._train_key, -1)
            cam_pos = {0: (0, 0), 6: (FRAME_WIDTH, 0),
                       2: (0, FRAME_HEIGHT), 4: (FRAME_WIDTH, FRAME_HEIGHT)}
            cx, cy = cam_pos.get(cam_idx, (0, 0))
            fw, fh = FRAME_WIDTH, FRAME_HEIGHT

            border_col = (0, 160, 255) if self._train_mode == "ready" else (0, 0, 255)
            cv2.rectangle(img, (cx, cy), (cx + fw, cy + fh), border_col, 5)

            bar_h = 76
            bar_y = cy + fh - bar_h
            ov = img.copy()
            cv2.rectangle(ov, (cx, bar_y), (cx + fw, cy + fh), (10, 10, 10), -1)
            cv2.addWeighted(ov, 0.82, img, 0.18, 0, img)

            s0, s1 = self._train_counts
            cv2.putText(img, f"stopped={s0}  running={s1}  total={s0+s1}",
                        (cx + 8, bar_y + 18), font, 0.45, (180, 180, 180), 1)

            if self._train_mode == "ready":
                cv2.putText(img, f"[ {self._train_key} ]",
                            (cx + 8, bar_y + 40), font, 0.5, (0, 200, 255), 1)
                cv2.putText(img, "1=STOPPED  2=RUNNING  ENTER=Entrenar  ESC=Salir",
                            (cx + 8, bar_y + 60), font, 0.38, (80, 255, 80), 1)
            else:
                lbl_str = "STOPPED" if self._train_label == 0 else "RUNNING"
                lbl_col = (80, 80, 255) if self._train_label == 0 else (80, 255, 80)
                blink   = int(now * 3) % 2 == 0
                dot     = "●" if blink else " "
                cv2.putText(img, f"{dot} REC  {lbl_str}",
                            (cx + 8, bar_y + 42), font, 0.65, lbl_col, 2)
                remain  = max(0.0, self._train_clip_end - now)
                prog    = 1.0 - remain / max(self._train_clip_sec, 1e-3)
                bar_x1  = cx + 8
                bar_x2  = cx + fw - 8
                cv2.rectangle(img, (bar_x1, bar_y + 54), (bar_x2, bar_y + 68), (45, 45, 45), -1)
                fill = bar_x1 + int((bar_x2 - bar_x1) * prog)
                cv2.rectangle(img, (bar_x1, bar_y + 54), (fill, bar_y + 68), lbl_col, -1)

        elif self._train_mode == "training":
            bw, bh = 400, 100
            bx = (w - bw) // 2
            by = (h - bh) // 2
            ov = img.copy()
            cv2.rectangle(ov, (bx, by), (bx + bw, by + bh), (15, 15, 15), -1)
            cv2.addWeighted(ov, 0.90, img, 0.10, 0, img)
            cv2.rectangle(img, (bx, by), (bx + bw, by + bh), (0, 200, 255), 2)
            dots = "." * (int(now * 2) % 4)
            cv2.putText(img, f"ENTRENANDO{dots}",
                        (bx + 30, by + 42), font, 0.9, (0, 220, 255), 2)
            cv2.putText(img, self._train_key or "",
                        (bx + 30, by + 72), font, 0.55, (160, 160, 160), 1)

    # ── Conveyor2 pick joints ─────────────────────────────────────

    def _compute_z_prepick(self):
        """Calcula z_prepick via FK subprocess al arrancar."""
        global Z_PREPICK_M
        try:
            joints_str = "[" + ", ".join(f"{j:.6f}" for j in PREPICK_JOINTS_C2S2) + "]"
            cmd = ["ros2", "service", "call",
                   "/robot2/niryo_robot/kinematics/forward",
                   "niryo_ned_ros2_interfaces/srv/GetFK",
                   f"joints: {joints_str}"]
            out = subprocess.run(cmd, capture_output=True, text=True, timeout=30)
            m = re.search(
                r"position=geometry_msgs\.msg\.Point\(x=([-\d.eE+]+),\s*y=([-\d.eE+]+),\s*z=([-\d.eE+]+)\)",
                out.stdout + out.stderr)
            if m:
                Z_PREPICK_M = float(m.group(3))
                self.get_logger().info(f"[C2S2] z_prepick calculado: {Z_PREPICK_M*1000:.1f}mm")
        except Exception as e:
            self.get_logger().warn(f"[C2S2] FK prepick falló ({e}), usando z_prepick=0.175m")

    def _on_niryo_factory_status(self, msg: String):
        try:
        ...
        self.get_logger().info(
            f"[C2S2] vision_robot2 SCANNING — command_id={command_id} — settle {C2S2_DYNAMIC_PICK_SETTLE_SEC}s")
        threading.Thread(
    ...
    def _call_ik(self, x: float, y: float, z: float) -> list:
        """Llama IK via CLI subprocess. Devuelve lista de 6 joints o None."""
        pose_str = (f"pose: {{position: {{x: {x:.6f}, y: {y:.6f}, z: {z:.6f}}}, "
                    f"rpy: {{roll: {ROLL_PICK:.4f}, pitch: {PITCH_PICK:.4f}, yaw: {YAW_PICK:.4f}}}}}")
        cmd = ["ros2", "service", "call",
               "/robot2/niryo_robot/kinematics/inverse",
               "niryo_ned_ros2_interfaces/srv/GetIK",
               pose_str]
        out = subprocess.run(cmd, capture_output=True, text=True, timeout=30)
        m = re.search(r"joints=\[([-\d.,\s eE+]+)\]", out.stdout + out.stderr)
        if not m:
            return None
        return [float(v.strip()) for v in m.group(1).split(",")]

    def _compute_robot2_c2s2_pick_after_settle(self, status: dict):
        try:
            time.sleep(C2S2_DYNAMIC_PICK_SETTLE_SEC)

            with self._lock:
                centroid = self._cam4_c2_centroid

            if centroid is None or self._H_c2 is None:
                self.get_logger().warn("[C2S2] Sin detección en conveyor2 tras settle — abortando")
                return

            px, py = centroid
            pt = self._H_c2 @ np.array([px, py, 1.0])
            pt /= pt[2]
            x_m = float(pt[0]) / 1000.0
            y_m = float(pt[1]) / 1000.0

            pick_joints    = self._call_ik(x_m, y_m, Z_PICK_M)
            prepick_joints = self._call_ik(x_m, y_m, Z_PREPICK_M)

            if pick_joints is None or prepick_joints is None:
                self.get_logger().warn("[C2S2] IK falló — usando joints fijos")
                pick_joints    = PICK_JOINTS_C2S2
                prepick_joints = PREPICK_JOINTS_C2S2

            payload = {
                "prepick_c2s2":       prepick_joints,
                "pick_c2s2":          pick_joints,
                "x_mm":               round(x_m * 1000, 2),
                "y_mm":               round(y_m * 1000, 2),
                "command_id":         status.get("command_id"),
            ...
            self._pick_joints_pub.publish(out)
            self.get_logger().info(
                f"[C2S2] publicado → ({x_m*1000:.1f}, {y_m*1000:.1f})mm "
                f"command_id={status.get('command_id')}")
        except Exception as e:
            self.get_logger().error(f"[C2S2] Error en settle worker: {e}")
